...
import math
import gensim
df = pd.read_csv('./feature_files/song_feature_file_full.csv')
...
def chord_all_w2v(data):
    ...
    #chord_score = calc_vec(chords[:,None],chords[None,:])
    return chord_score
...
#print("Calculating BPM similarity")
bpm_scores = bpm_all(df)
np.savetxt('bpm_scores.csv',bpm_scores)
#print("Generating BPM Graph")
G_bpm,bpm_communities = graph(df,bpm_scores,.96)
#del bpm_scores

print("Calculating Chord Similarity")
chord_scores = chord_all_w2v(df)
np.savetxt('chord_scores.csv',chord_scores)
#motion_scores = motion_all(df)



#print("Generating Chord Graph")
...
for i in bpm_communities:
    print(i)
print(len(bpm_communities))
#print("------------------------------------------------------------------------------")
#print('Chord Communities:')
#for i in chord_communities:
#    print(i)

#print("Spring Layouts")
